...
// Hour slots shown in the weekly table (index 1-12, matching hourObj keys)
...
// Reservation flow state (date / time / attribute filters chosen in the panel)
...
// Is the given hourIndex (1-12) free for this table on this date, taking
// permanent hours from earlier weeks into account (mirrors applyWeekData).
...
function openReservationModal(name) {
    ...
    nameInput.value = currentUser.isLoggedIn && currentUser.name ? currentUser.name : '';
...
function confirmReservation() {
    ...
    saveTimetable(name, t).then(() => {
        ...
        if (currentTimetableName === name) {
            updateTimetableForWeek(new Date(timetables[currentTimetableName].currentWeek));
        }
    });
}
